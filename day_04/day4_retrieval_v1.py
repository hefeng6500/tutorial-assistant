...
ALPHA = 0.5         # hybrid weight: final = alpha * vector_score + (1-alpha) * bm25_score (after norm)
# ----------------------------------------
...
def pipeline_query(llm, vectorstore, bm25, chunks, query, multi_q=3, top_n=TOP_K_VECTOR, alpha=ALPHA):
    ...
    return reranked  # list of (idx, score_llm)
...
def run_for_method(method_name, chunks, embeddings, queries, llm):
    ...
    for q in queries:
        print("\nQuery:", q)
        reranked = pipeline_query(llm, vectorstore, bm25, chunks, q, multi_q=3, top_n=TOP_K_VECTOR, alpha=ALPHA)
...
"""
        ...
        results.append({
            ...
            "used_sources": parsed.get("used_sources", used),
            "raw_llm": txt
        })
        # save per query
